...
/** Builds a deterministic, validated sub-mission without breaking the transit path between regions. */
object ActiveRecaptureMissionRegionFilter {
    @JvmStatic
    @JvmOverloads
    fun selectGroups(mission: SurveyMission, selectedGroupIds: Set<String>, context: Context? = null): SurveyMission {
        val groups = ActiveRecaptureMissionGroupCatalog.groups(mission, context)
        ...
        return select(mission, selectedRegionIds, context).copy(
            name = context?.getString(R.string.mission_selected_group_count, mission.name, selectedGroups.size, groups.size)
    ...
    @JvmStatic
    @JvmOverloads
    fun select(mission: SurveyMission, selectedRegionIds: Set<String>, context: Context? = null): SurveyMission {
        ...
        val selectedRegions = metadata.regions
            ...
            .map { region -> region.copy(passIndices = region.passIndices.mapNotNull(selectedPassIndexMap::get)) }
        val filteredPasses = groupPasses(filteredWaypoints)
        val photoCount = filteredPasses.sumOf(::captureCount)
        val surveyPhotoCount = selectedRegions.sumOf { it.suggestedSurveyPhotos }.coerceAtMost(photoCount)
        ...
        val filtered = mission.copy(
            ...
            name = context?.getString(R.string.mission_selected_group_count, mission.name, selectedRegions.size, metadata.regions.size)
    ...
    private fun boundingRoi(points: List<GeoPoint>): List<GeoPoint> {
        ...
        val longitudePadding = 5.0 / (111_320.0 * cos(Math.toRadians(centerLatitude)).coerceAtLeast(0.01))
    ...
    private fun distanceMeters(start: GeoPoint, end: GeoPoint): Double {
        ...
        val east = (end.longitude - start.longitude) * 111_320.0 * cos(Math.toRadians(meanLatitude))
