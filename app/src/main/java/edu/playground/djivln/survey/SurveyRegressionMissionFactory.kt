...
object SurveyRegressionMissionFactory {
    ...
    fun create(center: GeoPoint, fiveDirection: Boolean, context: Context? = null): SurveyMission {
        ...
        return SurveyPlanner.plan(
            name = if (fiveDirection) {
                context?.getString(R.string.debug_five_direction_regression_name) ?: "DEBUG five-direction regression 50x40m"
            } else {
                context?.getString(R.string.debug_nadir_regression_name) ?: "DEBUG nadir regression 50x40m"
            },
            roi = roi,
