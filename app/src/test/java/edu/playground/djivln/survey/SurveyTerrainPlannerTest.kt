...
class SurveyTerrainPlannerTest {
    @Test fun `terrain plan rejects a persisted reference different from its takeoff point`() {
        val roi = listOf(
            GeoPoint(31.0, 121.0), GeoPoint(31.0, 121.001),
            GeoPoint(31.001, 121.001), GeoPoint(31.001, 121.0),
        )
        val mission = SurveyPlanner.plan("mismatch", roi, takeoffPoint = roi.first())
        val terrain = object : TerrainElevationSource {
            override val info = TerrainRasterInfo(
                "constant", 2, 2, 4326, null, 1.0, 1.0,
                30.0, 32.0, 120.0, 122.0,
            )
            override fun elevationMeters(latitude: Double, longitude: Double) = 10.0
        }

        assertThrows(IllegalArgumentException::class.java) {
            SurveyTerrainPlanner.apply(
                mission, terrain, roi.first(), "a".repeat(64),
                takeoffReference = reference(roi.first().copy(latitude = 31.00001)),
            )
        }
    }
    ...
    fun `DSM creates intermediate height controls without restarting capture`() {
        ...
        val result = SurveyTerrainPlanner.apply(
            mission, terrain, roi.first(), "a".repeat(64),
            takeoffReference = reference(roi.first()),
        )
    ...
    private fun reference(point: GeoPoint) = SurveyTerrainTakeoffReference(
        point,
