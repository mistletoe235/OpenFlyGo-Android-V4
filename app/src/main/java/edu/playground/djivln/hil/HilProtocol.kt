...
object HilProtocol {
    ...
    fun encodeEvent(sessionId: Long, sequence: Long, value: Event): ByteArray {
        require(value.kind in EVENT_INFO..EVENT_EMERGENCY) { "invalid event kind" }
        require(!value.stopScore.isInfinite()) { "invalid event stop score" }
    ...
    fun decode(bytes: ByteArray, length: Int = bytes.size): Datagram {
        ...
        require(type in TYPE_HELLO..TYPE_EVENT) { "invalid HIL type" }
    ...
    fun decodeEvent(payload: ByteBuffer): Event {
        ...
        require(kind in EVENT_INFO..EVENT_EMERGENCY) { "invalid event kind" }
        val stopScore = payload.double
        require(!stopScore.isInfinite()) { "invalid event stop score" }
        ...
        val reason = runCatching {
            Charsets.UTF_8.newDecoder()
                ...
                .toString()
        }.getOrElse { throw IllegalArgumentException("invalid event reason", it) }
