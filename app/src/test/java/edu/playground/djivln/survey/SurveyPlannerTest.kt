...
class SurveyPlannerTest {
    ...
    @Test
    fun `all start corners preserve local ordering for split concave scanlines`() {
        ...
        val cornerModes = listOf(
            SurveyStartPointMode.FIRST_ROUTE_START, SurveyStartPointMode.ROUTE_CORNER_2,
        ...
        val cornerMissions = cornerModes.map { mode ->
            SurveyPlanner.plan(
                ...
                constraints = SurveyConstraints(
                    altitudeMetersAgl = 80.0,
                    routeHeadingDegrees = 90.0,
                    startPointMode = mode,
                ),
            ).also { mission ->
                val connectors = mission.surveyPasses().zipWithNext().map { (a, b) ->
                    distanceMeters(a.end.point, b.start.point)
                }
                assertTrue("$mode made a remote jump: ${connectors.maxOrNull()}",
                    connectors.maxOrNull()!! < 550.0)
            }
        }
        val reference = geo(-50.0, -50.0)
        val automatic = SurveyPlanner.plan(
            "split-auto", concave,
            constraints = SurveyConstraints(
                altitudeMetersAgl = 80.0,
                routeHeadingDegrees = 90.0,
        ...
        fun routeCost(value: SurveyMission) = distanceMeters(reference, value.waypoints.first().point) +
            value.surveyPasses().zipWithNext().sumOf { (a, b) ->
                distanceMeters(a.end.point, b.start.point)
            }
    ...
    fun `fixed nadir heading survives alternating passes and every start corner`() {
        for (collection in SurveyCollectionMode.values()) {
            for (start in SurveyStartPointMode.values()) {
                ...
                val expected = if (collection == SurveyCollectionMode.CROSSHATCH_NADIR) {
                    setOf(37, 127)
                } else setOf(37)
                assertEquals("$collection / $start", expected,
                ...
                assertEquals(tracking.waypoints.map { it.gimbalPitchDegrees },
                    fixed.waypoints.map { it.gimbalPitchDegrees })
    ...
    @Test
    fun `five direction planning and execution use independent route speeds`() {
        val constraints = SurveyConstraints(
        ...
        val mission = SurveyPlanner.plan("independent-speeds", rectangularRoi,
            constraints = constraints)
        val expected = SurveyPlanner.estimateRouteSeconds(mission.waypoints, constraints)

        assertEquals(2.0, constraints.speedForCaptureView(SurveyCaptureView.NADIR), 0.0)
        assertEquals(6.0, constraints.speedForCaptureView(SurveyCaptureView.LEFT_OBLIQUE), 0.0)
        assertEquals(6.0, constraints.maximumSurveySpeedMetersPerSecond, 0.0)
        assertEquals(expected, mission.estimatedFlightSeconds, 1.0e-9)
    }
