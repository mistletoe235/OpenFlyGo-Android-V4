...
object SurveyMissionJson {
    ...
    fun decode(raw: String): SurveyMission {
        ...
        if (root.has("execution_review")) {
            val review = root.optJSONObject("execution_review")
            require(review?.opt("safe_to_execute") == true) {
                "Mission execution review has not approved flight"
            }
        }
        ...
        return SurveyMission(
            ...
            terrainPlan = if (schemaVersion >= 7 && !root.isNull("terrain_plan")) {
                decodeTerrainPlan(root.getJSONObject("terrain_plan"))
            } else null,
    ...
    private fun decodeActiveMapping(value: JSONObject) = ActiveMappingMetadata(
        ...
        ordinaryGpsUsed = value.optBoolean("ordinary_gps_used", false),
    ...
    private fun decodeTerrainPlan(value: JSONObject) = SurveyTerrainPlan(
        ...
        takeoffReference = if (!value.isNull("takeoff_reference")) {
