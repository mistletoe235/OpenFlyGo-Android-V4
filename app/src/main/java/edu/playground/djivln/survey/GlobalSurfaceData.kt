...
class CompositeSurfaceElevationSource(
    ...
    init {
        require(info.minimumLatitude < info.maximumLatitude &&
            ...
            context?.getString(R.string.terrain_building_no_overlap)
                ?: "Terrain and building-height layers have no overlapping coverage"
        }
    }
    ...
    override fun elevationMeters(latitude: Double, longitude: Double): Double {
        ...
        require(buildingHeight >= 0.0) {
            context?.getString(R.string.building_relative_height_negative_value)
                ?: "Building relative height cannot be negative"
        }
