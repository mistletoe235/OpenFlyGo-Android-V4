...
/** User-facing grouping for a compiled active-recapture mission. */
object ActiveRecaptureMissionGroupCatalog {
    ...
    private val v30Definitions = listOf(
        ...
        Definition("V30_LARGE", 2, R.string.recapture_group_east_large_area, "East large-area five-direction", setOf("LARGE_FIVE_DIRECTION")),
        Definition("V30_SMALL_CROSS", 3, R.string.recapture_group_small_cross, "Small cross recapture", setOf("SMALL_CROSS")),
        Definition("V30_RISK_SCAN", 4, R.string.recapture_group_scattered_risk, "Scattered-risk recapture", setOf("V26_RISK_SCAN")),
    )
    ...
    @JvmStatic
    @JvmOverloads
    fun selectedGroupIds(source: SurveyMission, selected: SurveyMission, context: Context? = null): Set<String> {
        val selectedRegionIds = selected.activeMapping?.regions?.mapTo(hashSetOf()) { it.regionId }.orEmpty()
        return groups(source, context)
