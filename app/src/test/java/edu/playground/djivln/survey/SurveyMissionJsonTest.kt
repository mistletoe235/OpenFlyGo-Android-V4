...
class SurveyMissionJsonTest {
    @Test fun `unapproved execution review is rejected regardless of import source`() {
        val mission = SurveyPlanner.plan(name = "review", roi = listOf(
            GeoPoint(31.0, 121.0), GeoPoint(31.0, 121.001),
            GeoPoint(31.001, 121.001), GeoPoint(31.001, 121.0)))
        val root = JSONObject(SurveyMissionJson.encode(mission))
        for (review in listOf(JSONObject().put("safe_to_execute", false), JSONObject(), JSONObject.NULL)) {
            root.put("execution_review", review)
            assertTrue(runCatching { SurveyMissionJson.decode(root.toString()) }.isFailure)
        }
        root.put("execution_review", JSONObject().put("safe_to_execute", true))
        assertEquals(mission.id, SurveyMissionJson.decode(root.toString()).id)
    }

    @Test fun `terrain takeoff reference survives JSON round trip`() {
        ...
        ).copy(terrainPlan = SurveyTerrainPlan(
            sourceName = "test", sourceSha256 = "a".repeat(64), epsg = 4326,
            targetAglMeters = 30.0, takeoffTerrainElevationMeters = 8.0,
            sampleSpacingMeters = 3.0, minimumTerrainElevationMeters = 8.0,
            maximumTerrainElevationMeters = 10.0, minimumWaypointAltitudeMeters = 30.0,
            maximumWaypointAltitudeMeters = 32.0,
            takeoffReference = SurveyTerrainTakeoffReference(
                GeoPoint(31.0, 121.0), SurveyTerrainTakeoffReferenceSource.HOME_LOCATION, 123L),
        ))

        val decoded = SurveyMissionJson.decode(SurveyMissionJson.encode(mission))
        assertEquals(mission.terrainPlan, decoded.terrainPlan)
    }
    ...
    @Test
    fun `schema 13 stores independent speeds and older missions inherit safe defaults`() {
        val mission = SurveyPlanner.plan(
            name = "margin-schema",
            roi = listOf(
                GeoPoint(31.0000, 121.0000),
        ...
        assertEquals(schema11.constraints.speedMetersPerSecond,
            schema11.constraints.obliqueSpeedMetersPerSecond, 0.0)
        assertEquals(
            SurveyObliqueHeadingMode.TRACK_ROUTE,
            schema11.constraints.obliqueHeadingMode,
        )
