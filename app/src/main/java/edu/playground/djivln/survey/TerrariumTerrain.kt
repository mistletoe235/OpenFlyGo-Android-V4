...
object WebMercatorTileGrid {
    ...
    fun tileIdsForRoi(
        ...
        require(maxLon - minLon < 180.0) {
            context?.getString(R.string.roi_dateline_unsupported)
                ?: "Survey areas crossing the international date line are not supported"
        }
...
class GlobalTerrainDownloader(context: Context) {
    ...
    fun download(
        ...
        ids.forEachIndexed { index, id ->
            ...
            encodedBytes = TerrainImportSafety.requireAggregateWithinLimit(
                encodedBytes,
                bytes.size.toLong(),
                TerrainImportSafety.MAX_IMPORT_BYTES,
            ) {
    ...
    private fun fetch(id: WebMercatorTileId): Pair<ByteArray, String?> {
        ...
        return try {
            ...
            require(contentLength < 0L || contentLength <= MAXIMUM_TERRARIUM_TILE_BYTES) {
                appContext.getString(
                    R.string.terrain_file_mobile_memory_limit,
                    MAXIMUM_TERRARIUM_TILE_BYTES / (1024L * 1024L),
                )
            }
