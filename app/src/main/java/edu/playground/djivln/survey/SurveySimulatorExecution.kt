...
object SurveySimulatorGate {
    ...
    const val REAL_MIN_BATTERY_PERCENT = 30
    const val REAL_MIN_RC_BATTERY_PERCENT = 30
    ...
    fun evaluate(
        ...
        if (environment == SurveyExecutionEnvironment.DJI_SIMULATOR) {
            ...
            if (checkPreflightReadiness && telemetry.batteryPercent < REAL_MIN_BATTERY_PERCENT) {
                blocks += SurveyExecutionBlock.AIRCRAFT_BATTERY_LOW
            }
            if (checkPreflightReadiness && telemetry.rcBatteryPercent < REAL_MIN_RC_BATTERY_PERCENT) {
                blocks += SurveyExecutionBlock.RC_BATTERY_LOW
            }
...
class SurveySimulatorExecutionStateMachine(
    ...
    fun resume(gate: SurveyExecutionGateResult, currentPoint: GeoPoint?): SurveyExecutionStatus {
        return resume(gate, currentPoint, Double.NaN)
    }
    ...
    private fun buildResumeLegs(
        ...
        if (safeAltitude - currentPoint.altitudeMeters >
            SurveyWaypointFollower.VERTICAL_TOLERANCE_METERS
        ) {
        ...
        if (safeAltitude - recoveryPoint.altitudeMeters >
            SurveyWaypointFollower.VERTICAL_TOLERANCE_METERS
        ) {
            resumeLegs += recoveryLeg(
                recoveryPoint,
