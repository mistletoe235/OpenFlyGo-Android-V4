...
object SurveyTerrainPlanner {
    ...
    fun apply(
        ...
        require(horizontalDistance(takeoffPoint, takeoffReference.point) <= 0.1) {
            "Terrain takeoff point and persisted reference must match"
        }
        ...
        require(passes.sumOf { it.size } <= MAX_TERRAIN_CONTROL_POINTS) {
            context?.getString(R.string.dsm_control_points_exceeded, MAX_TERRAIN_CONTROL_POINTS)
                ?: "DSM route exceeds $MAX_TERRAIN_CONTROL_POINTS control points; reduce the survey area or flight-line density"
        }
        ...
        check(maximumVerticalSpeed <= safeVertical + 1e-6) {
            context?.getString(R.string.dsm_vertical_speed_envelope_error)
                ?: "DSM vertical-speed envelope calculation failed"
        }
