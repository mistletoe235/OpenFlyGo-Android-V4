...
2. 保存根地址与访问码，填写任务名、真实起飞点 ASL，创建会话并记下 ID。当前 V4 的 FOV / 型号
   从相机配置取得，最大补拍任务数在客户端固定为 10，不是该弹窗的可编辑项；先核对航线相机配置。
   不要把飞机显示的相对起飞高度填成绝对海拔。
...
5. 默认 schema 13 是停车稳定后拍照。V5“飞行 → 连续补拍（实验）”只针对主动补拍、需确认，
   ...
   V4 的具体条件和验证范围见 [schema 14 适配说明](SCHEMA14_CONTINUOUS_RECAPTURE_2026-09-22.md)。
