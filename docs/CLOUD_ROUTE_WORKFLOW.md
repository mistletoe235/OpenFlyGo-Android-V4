...
2. Save the root URL and access code, enter a task name and actual takeoff ASL, create a session and record its ID. V4 obtains FOV/model from the camera profile and fixes the maximum reacquisition task count at 10; these are not editable in that dialog. Check the profile first. Relative flight height is not ASL.
